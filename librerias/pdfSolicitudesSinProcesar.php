...
$consulta="select id_solicitud,concat(NOMBRE_USUARIO,' ',APELLIDO_USUARIO) AS USUARIO,descripcion,gerencia,sitio,
           motivo_solicitud.id_Motivo_Solicitud,motivo_solicitud.Motivo_Solicitud from solicitud_equipo
...
$pdf=new PDF_MC_Table();
$pdf->titulo('SOLICITUD SIN POROCESAR',$fecha);
$pdf->Open();
$pdf->AddPage();
$pdf->SetFont('Arial','',10);
$pdf->SetWidths(array(15,35,30,40,40,30));
$header=array('SOLICITUD','NOMBRES','DESCRIPCION','GERENCIA','SITIO','MOTIVO');
$swiche=0;
while ($row=mysql_fetch_array($result)){   
	   $pdf->Row($header,array($row[0],$row[1],$row[2],$row[3],$row[4],$row[6]),$swiche);
	   $swiche=1;
}
$pdf->Output();
?> 