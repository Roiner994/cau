...
if ($gerencia =='100' && $division=='100'){
   ...
   $consulta="select distinct requerimiento_sistema_informacion.id_software,software.software,
			  ...
			  and requerimiento_sistema_informacion.fecha_inicial between '$fechaI' and '$fechaF'
...
$pdf->encabezado('REQUERIMIENTOS DE SISTEMA DE INFORMACIÓN DE C.V.G VENALUM',"Ciudad Guayana, $fecha");
$pdf->Open();
$pdf->AddPage();
$pdf->SetFont('Arial','',10);
$pdf->SetWidths(array(80,80));
$header=array('SISTEMA DE INFORMACIÓN','CANTIDAD');
$swiche=0;
...
$consulta="select gerencia.gerencia,gerencia.id_gerencia,division.division,requerimiento_sistema_informacion.id_division,requerimiento_sistema_informacion.id_software,software.software,
	       sum(requerimiento_sistema_informacion.cantidad)
		   ...
		   requerimiento_sistema_informacion.fecha_inicial between '$fechaI' and '$fechaF'";
if ($gerencia=="" && $division=="" && $tipo==""){		
	$agrupar="group by gerencia.gerencia,requerimiento_sistema_informacion.id_software ORDER BY gerencia.gerencia";
...
$pdf=new PDF_MC_Table();
$pdf->titulo('GERENCIA DE SISTEMAS Y ORGANIZACIÓN','DIVISIÓN CENTRO DE ATENCIÓN A USUARIOS');
$pdf->encabezado('REQUERIMIENTOS SISTEMA DE INFORMACIÓN  POR GERENCIAS',"Ciudad Guayana, $fecha");
$pdf->Open();
$pdf->AddPage();
$pdf->SetFont('Arial','',10);
$pdf->SetWidths(array(60,60,60));
$header=array('GERENCIA','SISTEMA DE INFORMACIÓN','CANTIDAD');
$swiche=0;
while ($row=mysql_fetch_array($result)){   
...
mysql_close();
	...
	$agrupar="group by gerencia.gerencia,division.division,requerimiento_sistema_informacion.id_software ORDER BY gerencia.gerencia";	
	$consulta=$consulta.$agrupar;
	$result=mysql_query($consulta);
$fecha=getdate();
$fecha=$fecha[mday]."/".$fecha[mon]."/".$fecha[year];
...
$pdf=new PDF_MC_Table();
$pdf->titulo('GERENCIA DE SISTEMAS Y ORGANIZACIÓN','DIVISIÓN CENTRO DE ATENCIÓN A USUARIOS');
$pdf->encabezado('REQUERIMIENTOS DE SISTEMA DE INFORMACIÓN POR DIVISIONES',"Ciudad Guayana, $fecha");
$pdf->Open();
$pdf->AddPage();
$pdf->SetFont('Arial','',10);
$pdf->SetWidths(array(50,50,50,30));
$header=array('GERENCIA','DIVISION','SISTEMA DE INFORMACIÓN','CANTIDAD');
$swiche=0;
while ($row=mysql_fetch_array($result)){   
