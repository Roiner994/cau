...
<script language="javascript">
	...
	function generarExcel(){			
			...
			window.open('../librerias/rptPreventivoDetalladoExcel.php?anno='+anno+"&mes="+meses+"&sitio="+sitio);		
	}
	...
	function generarGrafico(){
		...
		meses=document.frmEquipo.mes.value;								
		sitio=document.frmEquipo.sitio.value;
		window.open('../librerias/rptPreventivoDetalladoGrafico.php?anno='+anno+"&mes="+meses+"&sitio="+sitio);		
...
<?php 
	...
	echo "<table class=\"formularioTabla\"align=center width=\"500\" border=\"0\">";
		...
		echo "<tr>";
		echo "<td class=\"tituloPagina\" colspan=\"2\">PREVENTIVO - REPORTE DETALLADO DE MANTENIMIENTOS</td>
  				</tr>";
			echo "<tr>";
...
<div width="70%">
	...
	<?php
		...
		$mes=isset($_POST['mes'])&&$_POST['mes']!=100 ? " AND MONTH(mantenimiento_preventivo.hora_inicio)=".$_POST['mes'] : "";	
	
		if(isset($_POST['anno']))
			$annp=$_POST['anno'];
		
		$anno=isset($_POST['anno'])&&$_POST['anno']!=100 ? "AND YEAR(mantenimiento_preventivo.hora_inicio)=".$_POST['anno'] : "";	
	...
	if(isset($_GET['meth'])){
		...
		"
			...
			$_pagi_cuantos = 31;//Elegí un número pequeño para que se generen varias páginas
		...
		if(!$_GET['_pagi_pg'])
			...
			while($row = mysql_fetch_array($_pagi_result)){
				...
				if($j!=$count-1)
					...
					echo "<td align=\"left\"> <a class=enlace href=\"#\" onclick=\"window.open('../librerias/detallePreventivoDetallado.php?dia=$row[0]&sitio=$sitio&mes=$mes&anno=$anno');\">$row[$j]</a>  </td>";				
					}
			...
			echo "<td align=\"left\"> <a class=enlace href=\"#\" onclick=\"window.open('../librerias/detallePreventivoDetallado.php?sitio=$sitio&mes=$mes&anno=$anno');\">$neto</a>  </td>";						
			echo "</tr>"; 
