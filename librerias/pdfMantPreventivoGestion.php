...
$consultaDistribucionMicros="select 
	sitio as EDIFICIO,
	count(if(id_descripcion='DES0000001',id_descripcion,null)) as TOTAL,
	count(if(critico='1' AND (id_descripcion='DES0000001'),critico,null)) as 'CRITICO',
	count(if(usuario_especializado='1' AND (id_descripcion='DES0000001'),usuario_especializado,null)) as 'USUARIO ESPECIALIZADO',
	count(if(red='0' AND (id_descripcion='DES0000001'),red,null)) as 'SIN RED',
	count(if(sistema_operativo='0' AND (id_descripcion='DES0000001'),sistema_operativo,null)) as 'SISTEMA OPERATIVO SIN ACTUALIZAR',
	count(if(correctivo='1' AND (id_descripcion='DES0000001'),correctivo,null)) as 'CORRECTIVOS'
...
$consultaDistribucionLaptops="select 
	sitio as EDIFICIO,
	count(if(id_descripcion='DES0000042',id_descripcion,null)) as TOTAL,
	count(if(critico='1' AND (id_descripcion='DES0000042'),critico,null)) as 'CRITICO',
	count(if(usuario_especializado='1' AND (id_descripcion='DES0000042'),usuario_especializado,null)) as 'USUARIO ESPECIALIZADO',
	count(if(red='0' AND (id_descripcion='DES0000042'),red,null)) as 'SIN RED',
	count(if(sistema_operativo='0' AND (id_descripcion='DES0000042'),sistema_operativo,null)) as 'SISTEMA OPERATIVO SIN ACTUALIZAR',
	count(if(correctivo='1' AND (id_descripcion='DES0000042'),correctivo,null)) as 'CORRECTIVOS'
	 from vistamantenimientosPreventivos where status_mantenimiento=2 and fecha_inicio between '$fechaInicial' and '$fechaFinal' group by sitio 
	order by sitio";

$consultaDistribucionImpresoras="select 
	sitio as EDIFICIO,
	count(if(id_descripcion='DES0000008',id_descripcion,null)) as TOTAL,
	count(if(critico='1' AND (id_descripcion='DES0000008'),critico,null)) as 'CRITICO',
	count(if(red='0' AND (id_descripcion='DES0000008'),red,null)) as 'SIN RED',
	count(if(correctivo='1' AND (id_descripcion='DES0000008'),correctivo,null)) as 'CORRECTIVOS'
	 from vistamantenimientosPreventivos where status_mantenimiento=2 and fecha_inicio between '$fechaInicial' and '$fechaFinal' group by sitio 
	order by sitio";
...
$resultResumenLaptos=mysql_query($consultaDistribucionLaptops);
$resultResumenImpresoras=mysql_query($consultaDistribucionImpresoras);
mysql_close();
...
$pdf->SetWidths(array(84,35,30,35,31,31,31,31));
$pdf->SetFont('Arial','B',10);
...
//Distribucion de MicroComputadoras
	...
	if ($resultResumenComputadoras && $resultResumenComputadoras!=1) {
		...
		$pdf->Row(array('TOTAL: ',$total[1],$total[2],$total[3],$total[4],$total[5],$total[6]));
	}
...
//DETALLE DE MICROCOMPUTADORES CRITICOS POR EDIFICIO

	$mantenimiento= new rptMantemiento();
	$detalleMantenimiento= new rptMantemiento();
	$mantenimiento->setFechas($fechaInicial,$fechaFinal);
	$mantenimiento->setDescripcion('DES0000001');
	$mantenimiento->setCritrico(1);
	$detalleMantenimiento->setDescripcion('DES0000001');
	$detalleMantenimiento->setCritrico(1);
	$Sitios=$mantenimiento->retornaMantenimientos('id_sitio','sitio');
	$detalleMantenimiento->setFechas($fechaInicial,$fechaFinal);
	...
	if ($Sitios && $Sitios!=1) {
		...
		while($row=mysql_fetch_array($Sitios)) {
			$pdf->SetWidths(array(45,25,90,30));
			$pdf->SetFont('Arial','B',10);
			$pdf->tituloTabla("MICROCOMPUTADORAS CON ESTADO CRITICO EN ".$row[31]);
			$detalleMantenimiento->setSitio($row[30]);
			$resultadoDetalle=$detalleMantenimiento->retornaMantenimientos();
			$pdf->RowColumnas(array('DESCRIPCION','CONF','USUARIO','EXTENSION'));
			$pdf->SetFont('Arial','',10);
...
//DETALLE DE MICROCOMPUTADORES USUARIOS ESPECIALIZADOS POR EDIFICIO

	$mantenimiento= new rptMantemiento();
	$detalleMantenimiento= new rptMantemiento();
	$mantenimiento->setFechas($fechaInicial,$fechaFinal);
	$mantenimiento->setDescripcion('DES0000001');
	$mantenimiento->setUsuarioEspecializado('1');
	$detalleMantenimiento->setDescripcion('DES0000001');
	$detalleMantenimiento->setUsuarioEspecializado('1');
	$Sitios=$mantenimiento->retornaMantenimientos('id_sitio','sitio');
	$detalleMantenimiento->setFechas($fechaInicial,$fechaFinal);
	...
	if ($Sitios && $Sitios!=1) {
		...
		while($row=mysql_fetch_array($Sitios)) {
			$pdf->SetWidths(array(45,25,90,30));
			$pdf->SetFont('Arial','B',10);
			$pdf->tituloTabla("MICROCOMPUTADORAS CON USUARIOS ESPECIALIZADOS EN ".$row[31]);
			$detalleMantenimiento->setSitio($row[30]);
			$resultadoDetalle=$detalleMantenimiento->retornaMantenimientos();
			$pdf->RowColumnas(array('DESCRIPCION','CONF','USUARIO','EXTENSION'));
			$pdf->SetFont('Arial','',10);
...
//DETALLE DE MICROCOMPUTADORES SIN RED POR EDIFICIO

	$mantenimiento= new rptMantemiento();
	$detalleMantenimiento= new rptMantemiento();
	$mantenimiento->setFechas($fechaInicial,$fechaFinal);
	$mantenimiento->setDescripcion('DES0000001');
	$mantenimiento->setRed(0);
	$detalleMantenimiento->setDescripcion('DES0000001');
	$detalleMantenimiento->setRed(0);
	$Sitios=$mantenimiento->retornaMantenimientos('id_sitio','sitio');
	$detalleMantenimiento->setFechas($fechaInicial,$fechaFinal);
	...
	if ($Sitios && $Sitios!=1) {
		...
		while($row=mysql_fetch_array($Sitios)) {
			$pdf->SetWidths(array(45,25,90,30));
			$pdf->SetFont('Arial','B',10);
			$pdf->tituloTabla("MICROCOMPUTADORAS SIN CONEXION A RED EN ".$row[31]);
			$detalleMantenimiento->setSitio($row[30]);
			$resultadoDetalle=$detalleMantenimiento->retornaMantenimientos();
			$pdf->RowColumnas(array('DESCRIPCION','CONF','USUARIO','EXTENSION'));
			$pdf->SetFont('Arial','',10);
...
//DETALLE DE MICROCOMPUTADORES CON CORRECTIVOS POR EDIFICIO

	$mantenimiento= new rptMantemiento();
	$detalleMantenimiento= new rptMantemiento();
	$mantenimiento->setFechas($fechaInicial,$fechaFinal);
	$mantenimiento->setDescripcion('DES0000001');
	$mantenimiento->setCorrectivo(1);
	$detalleMantenimiento->setDescripcion('DES0000001');
	$detalleMantenimiento->setCorrectivo(1);
	$Sitios=$mantenimiento->retornaMantenimientos('id_sitio','sitio');
	$detalleMantenimiento->setFechas($fechaInicial,$fechaFinal);
	...
	if ($Sitios && $Sitios!=1) {
		...
		$pdf->AddPage();
		while($row=mysql_fetch_array($Sitios)) {
			$pdf->SetWidths(array(25,50,25,90));
			$pdf->SetFont('Arial','B',10);
			$pdf->tituloTabla("MICROCOMPUTADORAS CON MANT. CORRECTIVOS EN ".$row[31]);
			$detalleMantenimiento->setSitio($row[30]);
			$resultadoDetalle=$detalleMantenimiento->retornaMantenimientos();
			$pdf->RowColumnas(array('CONF','USUARIO','EXTENSION','TRABAJO REALIZADO'));
			$pdf->SetFont('Arial','',10);
			if ($resultadoDetalle && $resultadoDetalle!=1)
				while ($rowEquipos=mysql_fetch_array($resultadoDetalle)) {
					$pdf->Row(array($rowEquipos[1],$rowEquipos[19].' '.$rowEquipos[20],$rowEquipos[21],strtoupper($rowEquipos[41])));
				}
				$pdf->Ln(5);			
...
//------------------------- Distribucion de LAPTOPS---------------------------------------------
$pdf->setOrientacion('L');
$pdf->AddPage('L');
$pdf->AliasNbPages();
$pdf->SetFont('Arial','B',10);
$pdf->tituloTabla('RESUMEN DE MANTENIMIENTOS PREVENTIVOS DE LAPTOPS');
$pdf->SetWidths(array(84,35,30,35,31,31,31,31));
$pdf->SetFont('Arial','B',10);
...
unset($Nombre);
unset($total);
for ($i=0;$i<mysql_numfields($resultResumenLaptos);$i++)
	$Nombre[$i]=mysql_field_name($resultResumenLaptos,$i);
...
//Distribucion de LAPTOPS
	...
	if ($resultResumenLaptos && $resultResumenLaptos!=1) {
		while($row=mysql_fetch_array($resultResumenLaptos)) {
			if ($row[1]>0)
			$pdf->Row(array($row[0],$row[1],$row[2],$row[3],$row[4],$row[5],$row[6]));
			$total[1]=$total[1]+$row[1];
			$total[2]=$total[2]+$row[2];
...
//DETALLE DE LAPTOPS CRITICOS POR EDIFICIO
	...
	if ($Sitios && $Sitios!=1) {
		...
		while($row=mysql_fetch_array($Sitios)) {
			$pdf->SetWidths(array(45,25,90,30));
			$pdf->SetFont('Arial','B',10);
			$pdf->tituloTabla("LAPTOPS CON ESTADO CRITICO EN ".$row[31]);
			$detalleMantenimiento->setSitio($row[30]);
			$resultadoDetalle=$detalleMantenimiento->retornaMantenimientos();
			$pdf->RowColumnas(array('DESCRIPCION','CONF','USUARIO','EXTENSION'));
			$pdf->SetFont('Arial','',10);
...
//DETALLE DE LAPTOPS USUARIOS ESPECIALIZADOS POR EDIFICIO
	...
	if ($Sitios && $Sitios!=1) {
		...
		while($row=mysql_fetch_array($Sitios)) {
			$pdf->SetWidths(array(45,25,90,30));
			$pdf->SetFont('Arial','B',10);
			$pdf->tituloTabla("LAPTOPS CON USUARIOS ESPECIALIZADOS EN ".$row[31]);
			$detalleMantenimiento->setSitio($row[30]);
			$resultadoDetalle=$detalleMantenimiento->retornaMantenimientos();
			$pdf->RowColumnas(array('DESCRIPCION','CONF','USUARIO','EXTENSION'));
			$pdf->SetFont('Arial','',10);
...
//DETALLE DE LAPTOPS SIN RED POR EDIFICIO
	...
	if ($Sitios && $Sitios!=1) {
		...
		while($row=mysql_fetch_array($Sitios)) {
			$pdf->SetWidths(array(45,25,90,30));
			$pdf->SetFont('Arial','B',10);
			$pdf->tituloTabla("LAPTOPS SIN CONEXION A RED EN ".$row[31]);
			$detalleMantenimiento->setSitio($row[30]);
			$resultadoDetalle=$detalleMantenimiento->retornaMantenimientos();
			$pdf->RowColumnas(array('DESCRIPCION','CONF','USUARIO','EXTENSION'));
			$pdf->SetFont('Arial','',10);
...
//DETALLE DE LAPTOPS CON CORRECTIVOS POR EDIFICIO
	...
	if ($Sitios && $Sitios!=1) {
		...
		while($row=mysql_fetch_array($Sitios)) {
			$pdf->SetWidths(array(25,50,25,90));
			$pdf->SetFont('Arial','B',10);
			$pdf->tituloTabla("LAPTOPS CON MANT. CORRECTIVOS EN ".$row[31]);
			$detalleMantenimiento->setSitio($row[30]);
			$resultadoDetalle=$detalleMantenimiento->retornaMantenimientos();
			$pdf->RowColumnas(array('CONF','USUARIO','EXTENSION','TRABAJO REALIZADO'));
			$pdf->SetFont('Arial','',10);
...
//------------------------- Distribucion de IMPRESORAS---------------------------------------------
$pdf->setOrientacion('F');
$pdf->AddPage();
$pdf->AliasNbPages();
$pdf->SetFont('Arial','B',10);
$pdf->tituloTabla('RESUMEN DE MANTENIMIENTOS PREVENTIVOS DE IMPRESORAS');
$pdf->SetWidths(array(69,35,30,25,31));
$pdf->SetFont('Arial','B',10);
...
unset($Nombre);
unset($total);
for ($i=0;$i<mysql_numfields($resultResumenImpresoras);$i++)
	$Nombre[$i]=mysql_field_name($resultResumenImpresoras,$i);
...
//Distribucion de IMPRESORAS
	...
	if ($resultResumenImpresoras && $resultResumenImpresoras!=1) {
		while($row=mysql_fetch_array($resultResumenImpresoras)) {
			if ($row[1]>0)
...
//DETALLE DE LAPTOPS CRITICOS POR EDIFICIO
	...
	if ($Sitios && $Sitios!=1) {
		...
		while($row=mysql_fetch_array($Sitios)) {
			$pdf->SetWidths(array(45,25,90,30));
			$pdf->SetFont('Arial','B',10);
			$pdf->tituloTabla("IMPRESORAS CON ESTADO CRITICO EN ".$row[31]);
			$detalleMantenimiento->setSitio($row[30]);
			$resultadoDetalle=$detalleMantenimiento->retornaMantenimientos();
			$pdf->RowColumnas(array('DESCRIPCION','CONF','USUARIO','EXTENSION'));
			$pdf->SetFont('Arial','',10);
...
//DETALLE DE IMPRESORAS CON CORRECTIVOS POR EDIFICIO
	...
	if ($Sitios && $Sitios!=1) {
		...
		while($row=mysql_fetch_array($Sitios)) {
			$pdf->SetWidths(array(25,50,25,90));
			$pdf->SetFont('Arial','B',10);
			$pdf->tituloTabla("IMPRESORAS CON MANT. CORRECTIVOS EN ".$row[31]);
			$detalleMantenimiento->setSitio($row[30]);
			$resultadoDetalle=$detalleMantenimiento->retornaMantenimientos();
			$pdf->RowColumnas(array('CONF','USUARIO','EXTENSION','TRABAJO REALIZADO'));
			$pdf->SetFont('Arial','',10);
