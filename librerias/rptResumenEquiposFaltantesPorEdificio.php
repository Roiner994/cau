...
<?php
	...
	GROUP BY SITIO";	
		...
		$_pagi_propagar = array("anno","sitio","item");
		...
		while($row = mysql_fetch_array($_pagi_result)){
			...
			for ($j = 1; $j < $count; $j++)
				...
				echo "<td align=\"left\"> <a class=enlace href=\"#\" onclick=\"window.open('detalleEquiposPendientesExcel.php?sitio=$row[0]&anno=$anno')\">$row[$j]</a>  </td>";
				$neto+=($row[$j]+0);
				}
		...
		echo "<td align=\"left\"> <a class=enlace href=\"#\" onclick=\"window.open('detalleEquiposPendientesExcel.php?sitio=$row[0]&anno=$anno&total=$neto')\">$neto</a> </td>";					
		echo "</tr>";
