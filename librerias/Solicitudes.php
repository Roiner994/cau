...
<script>
 function actualizar(){	  
	...
	//<body onUnload=JavaScript:actualizar(); topmargin=0> 
 }
</script> 	
<?PHP
...
switch ($_POST[funcion]) {
	case "1":
	   if ($_POST[selStatus]==100){
		...
		break 1;
	case "2":
	datoSolicitud($ficha,$idSolicitud,$estatus,$gerencia,$division,$departamento,$estuSolicitud,$login);	
	break 1;	
	default:
	datoSolicitud($ficha,$idSolicitud,$estatus,$gerencia,$division,$departamento,$estuSolicitud,$login);		   
}

function datoSolicitud($ficha,$idSolicitud,$estatus,$gerencia,$division,$departamento,$estuSolicitud,$login){		
//inicio de tabla
 echo "<form name=\"frmSolicitudes\" method=\"post\" action=\"\">";
 echo "<input name=\"funcion\" type=\"hidden\" value=\"1\">";     
	...
	where status_solicitud.id_status ='$estatus' and solicitud_equipo.id_solicitud='$idSolicitud' and gerencia.id_gerencia='$gerencia'
    ...
    //consulta que muestra los diferentes status que puede tener una solicitud			
	$consultaStatus1 = "SELECT id_status, des_status FROM status_solicitud where ID_STATUS='STA0000003' OR ID_STATUS='STA0000004' OR id_status='STA0000005' ORDER BY status_solicitud.des_status";
    ...
    echo "<td class=\"formularioTablaTitulo\">FECHA: $fecha<br><br>DATOS DE SOLICITUD<td class=formularioTablaSolicitud valign=top align=right><b>SOLICITUD Nº: $idSolicitud</b></td></tr>";
    echo "<tr>";  
    echo "<input name=\"ficha\" type=\"hidden\" value=\"$_POST[ficha]\">";    
	echo "<input name=\"selStatus\" type=\"hidden\" value=\"$_POST[selStatus]\">";
	echo "<input name=\"observEstatus\" type=\"hidden\" value=\"$_POST[observEstatus]\">";	       
	echo "<td class=\"formularioCampoTitulo\">FICHA<td>
	      <input name=\"ficha\" class=\"formularioCampo\" size=\"65\" maxlength=\"65\" readonly=\"true\" type=\"text\" value=\"$ficha\"><TR>";	
	...
	/*if ($estuSolicitud!='STA0000002'){//RECHAZADO
	    echo "<td class=\"formularioCampoTitulo\">OBSERVACION<td>
	          <textarea name=\"observ\" class=\"formularioCampo\" readonly=\"true\" cols=\"67\" rows=\"3\">$row[6]</textarea><tr>";		
	} */   
	if($estuSolicitud=='STA0000006'){// SIN PROCESAR	  	  
	...
	if($estuSolicitud=='STA0000003'){//EN PROCESO
	   $consultaStatus1 = "SELECT id_status, des_status FROM status_solicitud where ID_STATUS='STA0000004' OR id_status='STA0000005' ORDER BY status_solicitud.des_status";
	   $conSelEstatus1= new campoSeleccion("selStatus","formularioCampoSeleccion","$_POST[selStatus]","onChange","",$consultaStatus1,"--SELECCIONE--","");
	   $selEstatus1=$conSelEstatus1->retornar();		  	  
       echo "<td class=\"formularioCampoTitulo\">ESTATUS SOLICITUD</td>
             <td>$selEstatus1</td><tr>";       
	} 
	if($estuSolicitud=='STA0000001'){//APROBADO	  	  
      echo "<td class=\"formularioCampoTitulo\">ESTATUS SOLICITUD</td>
            <td>$selEstatus1</td><tr>";       
	}
   if($estuSolicitud!='STA0000004' && $estuSolicitud!='STA0000002'  && $estuSolicitud!='STA0000005'){//EJECUTADO O RECHAZADO     
     echo "<b><td class=\"formularioCampoTitulo\">OBSERVACION ESTATUS</td>"."<td>"; 	
     echo "<textarea name=\"observEstatus\" class=\"formularioCampo\"  cols=\"67\" rows=\"3\"></textarea><tr>";		    				         
     echo "<input=\"status\" tipe=\"hidden\" value=\"$_POST[selStatus]\">";
   }  
   if($estuSolicitud=='STA0000001'|| $estuSolicitud=='STA0000004' || $estuSolicitud=='STA0000002' || $estuSolicitud=='STA0000003' || $estuSolicitud=='STA0000005'|| $estuSolicitud=='STA0000006'){	//APROBADO O EJECUTADO O RECHAZADO O EN PROCESO O EJECUTADO SIN PUNTO DE RED O SIN PROCESAR       
   ...
   echo "<TABLE width=\"500\" border=0>      
         ...
         <input name=\"btnAlmacenar\" type=\"button\" class=\"Estilo71\" value=\"ACTUALIZAR\" onclick=\"actualizar();\"></td>";  
   echo "</TABLE>";		
   echo "</CENTER>";
   }
   if($estuSolicitud!='STA0000004' && $estuSolicitud=='STA0000002'){//EJECUTADO O RECHAZADO	  	        
   echo "<CENTER>";
   echo "<TABLE width=\"500\" border=0>      
         <TD class=\"formularioTablaBotones\">   
         <input name=\"btnAlmacenar\" type=\"button\" class=\"Estilo71\" value=\"ACTUALIZAR\" onclick=\"actualizar();\"></td>";  
