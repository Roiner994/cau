...
if ($swiche==0){    
    ...
    $this->SetFont('Arial','B',6);
    //Cabecera  
	$w=array(15,20,25,10,35,15,40,35); 
...
$pdf->titulo('SOLICITUDES CARGADAS',$fecha);
$pdf->Open();
$pdf->AddPage();
$pdf->SetFont('Arial','',10);
$pdf->SetWidths(array(15,20,25,10,35,15,40,35));
//Títulos de las columnas
$header=array('SOLICITUD','ESTATUS','EQUIPO','FICHA','USUARIO','EXTENSIÓN','GERENCIA','SITIO');
//Carga de datos
$swiche=0;
while ($row=mysql_fetch_array($result)){   
	   $pdf->Row($header,array($row[0],$row[1],$row[2],$row[3],$row[4],$row[5],$row[6],$row[7]),$swiche);
	   $swiche=1;
}
$pdf->Output();
?> 