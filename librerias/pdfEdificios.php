...
$pdf->titulo('SOLICITUD POR EDIFICIO',$fecha);
$pdf->Open();
$pdf->AddPage();
$pdf->SetFont('Arial','',10);
$pdf->SetWidths(array(15,30,40,15,45,15,25));
$header=array('SOLICITUD','EQUIPO','SITIO','FICHA','USUARIO','EXTENSIÓN','ESTATUS');
$swiche=0;
while ($row=mysql_fetch_array($result)){   
	   $pdf->Row($header,array($row[1],$row[7],$row[0],$row[4],$row[5],$row[6],$row[2]),$swiche);
	   $swiche=1;
}
$pdf->Output();
?> 