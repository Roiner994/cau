...
require_once ("inventarioAdmin.php");
	...
	conectarMysql();
	if(isset($_GET[sitio])&&isset($_GET[mes]))	{
		...
		if(isset($_GET[meses])){			
			$mes=isset($_GET['meses'])&&$_GET['meses']!=100  ? " AND MONTH(FECHA_INICIO)='".$_GET['meses']."'" : "";			
			$sitio=isset($_GET['sitio'])&&$_GET['sitio']!=100  ? " AND ID_SITIO='".$_GET['sitio']."'" : "";			
			$anno=isset($_GET['anno'])&&$_GET['anno']!=100  ? " AND YEAR(FECHA_INICIO)='".$_GET['anno']."'" : "";					
			$query="SELECT * FROM vistamantenimientospreventivos WHERE TRUE $mes $anno $sitio";												
		}
