...
<script type="text/javascript">
	...
	function generarReporte(idDescripcion,idConfiguracion){
		...
		window.open('../librerias/rptResumenComponenteporEquipo.php?idDescripcion='+idDescripcion+'&idConfiguracion='+idConfiguracion);	
	}

	function generarExcel() {
		idDescripcion=document.frmInventarioComponentesEquipo.selDescripcion.value;
		
		window.open('../librerias/rptResumenComponenteporEquipoExcel.php?idDescripcion='+idDescripcion);
	}
...
function frmInventarioComponentesEquipo() {
	...
	require_once("rptComponenteEquipo.php");
	...
	$selDescripcion=$descripcion->retornar();
		...
		<input name=\"btnLimpiar\" type=\"button\" value=\"LIMPIAR\" onclick=\"location.href='index2.php?item=631'\"></td>		
		</tr>";
		echo "</table>";
	 			...
	 			if ($resultado && $resultado!=1) {
						...
						<td valign=top class=\"tablaTitulo\"><b>CANTIDAD</b></td>		  
	 					...
	 					while ($row=mysql_fetch_array($resultado)) {
	 						...
	 						echo "<tr class=\"$clase\">";
	 							echo "<td align=\"left\"><a class=enlace href=\"#\" onclick=\"generarReporte('$_POST[selDescripcion]','$row[8]')\"> $row[9]</a></td>";
