	...
	$resultado=$estatus->cambiarStatusGarantia();
		switch($resultado){
					case 0:	
						...
						echo "<tr>";
						echo "<td valign=top class=\"mensaje\" align=center><input name=\"btnLimpiar\" type=\"button\" value=\"IMPRIMIR\" onclick=\"location.href='secciones.php?item=501'\"></td>
  						</tr>";
						echo "</table>";
						echo "</form>";	
						break 1;
...
if ($_POST[selProveedor]==100) {
	...
	$garantiaI=$_POST[selProveedor];
		$despacho=new garantia($_POST[chkGarantia],"",$_POST[selStatus],"","","","","","",$_POST[txtSerial],$_POST[selProveedor]);
		$resultado=$despacho->equipoFueraPlantaPrueba() ;
		$total=$despacho->total();
		$row=mysql_fetch_array($resultado);
...
echo "<table width=\"700\" border=\"0\" align=\"center\">
		...
		while($row=mysql_fetch_array($resultado)) {
			...
			if ($_POST[selStatus]=='STG0000003') {
				echo "<a class=enlace href=\"secciones.php?item=507&serial=$row[2]\">$row[2]</td>";
			} else {
				echo "<input name=\"chkGarantia[]\" type=\"checkbox\" value=\"$row[15]\">$row[2]</td>";
			}
			echo "<td>$row[7]</td>				
				<td>$row[6]</td>
				<td>$row[8]</td>
		...
		echo "<tr class=\"$clase\">			    
				<td>";
			if ($_POST[selStatus]=='STG0000003') {
				echo "<a class=enlace href=\"secciones.php?item=505&serial=$row[2]\">$row[2]</td>";
			} else {
				echo "<input name=\"chkGarantia[]\" type=\"checkbox\" value=\"$row[15]\">$row[2]</td>";
			}
			echo "<td>$row[7]</td>				
