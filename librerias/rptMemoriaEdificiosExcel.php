...
require_once ("inventarioAdmin.php");
	...
	$query="SELECT usuario_sistema.NOMBRE ,usuario_sistema.ID_USS
		...
		GROUP BY usuario_sistema.ID_USS";		
				...
				$query="SELECT CONCAT(modelo.CAP_VEL,' ',modelo.UNIDAD,' ',tipo.TIPO) FROM 
...
modelo.ID_DESCRIPCION='DES0000010' $sitio GROUP BY  CONCAT(modelo.CAP_VEL,' ',modelo.UNIDAD,' ',tipo.TIPO)";
	...
	for($i=0;$i!=$lm;$i++){
		$tecnicos.=" , COUNT(
					CASE (CONCAT(modelo.CAP_VEL,' ',modelo.UNIDAD,' ',tipo.TIPO))
					WHEN '$tid[$i]' THEN 1                           
					END        
					)AS `$tid[$i]` ";
		$total.="+COUNT(
...
sitio.SITIO!='OTROS' AND
sitio.SITIO!='DEPOSITO' AND
modelo.ID_DESCRIPCION='DES0000010' $sitio  GROUP BY 	equipo_componente_campo.CONFIGURACION";	
