...
function buscar_gerencia(){ 
	 ...
	 $cuentaConsulta="SELECT  count(id_solicitud) as total from solicitud_equipo inner join ubicacion on solicitud_equipo.id_ubicacion=ubicacion.id_ubicacion inner join gerencia on ubicacion.id_gerencia=gerencia.id_gerencia ";
     ...
     $consulta= "select id_solicitud,gerencia,descripcion,fecha_i,des_status from solicitud_equipo inner join status_solicitud on solicitud_equipo.id_status=status_solicitud.id_status inner join ubicacion on solicitud_equipo.id_ubicacion=ubicacion.id_ubicacion inner join descripcion on solicitud_equipo.id_descripcion=descripcion.id_descripcion inner join gerencia on ubicacion.id_gerencia=gerencia.id_gerencia ";		        
	  ...
	  while($row1=mysql_fetch_array($result)){
	     equiposEncontrados($row1[0],$row1[4],$row1[2],$row1[1],$row1[3]);       	               
	  }	
...
function inicioTabla() {
    ...
    echo "<TR vAlign=\"top\">";
    echo "<td width=\"680\" class=\"Estilo74\" align=\"left\" background=\"bgimage_over.jpg\">";
    echo"<b>TABLA DE SOLICITUD DE GERENCIA</b></td>";
    echo "</table>";    
	echo "<table width=\"683\" border=\"1\" cellpadding=\"0\" cellspacing=\"0\" align=\"center\">".
	"<tr class=\"Estilo10\">".
	...
	"<th width=\"100\"  scope=\"col\">FECHA DE SOLICITUD</th>";
	echo "</tr>";
...
function equiposEncontrados($Solicitud,$Status,$Equipo,$Gerencia,$Fecha) {    
	echo "<tr>";
	echo "<td align=\"center\" valign=\"middle\" class=\"Estilo71\"><a href=\"index.php?item=11&conf=$Solicitud\">$Solicitud</a></td>";	
...
function inicioTabla1() {       
    ...
    echo "<TR vAlign=\"top\">";
    echo "<td width=\"680\" class=\"Estilo74\" align=\"left\" background=\"bgimage_over.jpg\">";
    echo"<b>TABLA: TOTAL DE SOLICITUD DE GERENCIA</b></td>";
    echo "</table>";	   
	echo "<table width=\"683\" border=\"1\" cellpadding=\"0\" cellspacing=\"0\" align=\"center\">".
	"<tr class=\"Estilo10\">".
	"<th width=\"100\"  bgcolor=\"#99CC99\" scope=\"col\">TOTAL  SOLICITUD</th>".
	"<th width=\"100\"  bgcolor=\"#0000FF\" scope=\"col\">SOLICITUD SIN PROCESAR</th>".
	"<th width=\"100\"  bgcolor=\"#0000FF\" scope=\"col\">SOLICITUD RECHAZADA</th>".
	"<th width=\"100\"  bgcolor=\"#99CC99\" scope=\"col\">SOLICITUD APROBADA</th>".
	"<th width=\"100\"  bgcolor=\"#0000FF\" scope=\"col\">SOLICITUD EN PROCESO</th>".
	"<th width=\"100\"  bgcolor=\"#0000FF\"scope=\"col\">SOLICITUD EJECUTADA</th>".
	"<th width=\"100\"  bgcolor=\"#0000FF\"scope=\"col\">SOLICITUD EJECUTADA/SIN PTO RED</th>";
	echo "</tr>";
...
conectarMysql();   
    $busca= "SELECT solicitud_equipo.FICHA,NOMBRE_USUARIO,EXTENSION,CARGO,GERENCIA,DEPARTAMENTO,DIVISION,SITIO
   ...
   echo "<input name=\"Volver\" aling=\"center\" type=\"button\" value=\"Volver\" onClick=\"history.go(-1)\" >";
   echo "</form>";
...
function inicioTabla2() {
	echo "<table width=\"500\" border=\"1\" cellpadding=\"0\" cellspacing=\"0\" align=\"center\">".
