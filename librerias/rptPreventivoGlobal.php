...
<script language="javascript">
	...
	function generarExcel(){			
			...
			window.open('../librerias/rptPreventivoGlobalExcel.php?anno='+anno+"&mes="+meses);		
	}
	
	function generarGrafico(){
		anno=document.frmEquipo.anno.value;
		meses=document.frmEquipo.mes.value;
		window.open('../librerias/rptPreventivoGlobalGrafico.php?anno='+anno+"&mes="+meses);		
	...
	function generarReporte(){					
			anno=document.frmEquipo.anno.value;
			meses=document.frmEquipo.mes.value;					
			window.open('../librerias/rptResumenPreventivoGlobal.php?anno='+anno+"&mes="+meses);		
	}
</script>
...
<?php 
	...
	echo "<table class=\"formularioTabla\"align=center width=\"500\" border=\"0\">";
		...
		echo "<tr>";
		echo "<td class=\"tituloPagina\" colspan=\"2\">PREVENTIVO - REPORTE GLOBAL DE MANTENIMIENTOS</td>
  				</tr>";
			echo "<tr>";
		...
		/*SELECT equipo_campo.CONFIGURACION, modelo.CAP_VEL
FROM equipo_campo, modelo, inventario, inventario_ubicacion,sitio
WHERE equipo_campo.ID_INVENTARIO=inventario.ID_INVENTARIO
AND inventario.ID_MODELO=modelo.ID_MODELO
AND inventario_ubicacion.ID_INVENTARIO=inventario.ID_INVENTARIO
AND inventario_ubicacion.ID_SITIO=SITIO.ID_SITIO*/
?>
...
<?php
	...
	$anno=isset($_POST['anno'])&&$_POST['anno']!=100 ? " WHERE ANNO=".$_POST['anno'] : "";
	if ($_POST['mes']==100){
...
//realizamos el conteo alternativo.
	...
	$_pagi_propagar = array("mes","anno","item","ordenado","ordentipo","meth","mes_value");
	...
	if($mes!=100){
		...
		while($row = mysql_fetch_array($_pagi_result)){
			...
			echo "<td align=\"left\"> <a class=enlace href=\"#\" onclick=\"window.open('../librerias/detallePreventivoGlobal.php?meses=$mes_value&anno=$anno&sitio=$row[3]')\">$row[2]</a>  </td>";	
		...
		echo "<td align=\"left\"> <a class=enlace href=\"#\" onclick=\"window.open('../librerias/detallePreventivoGlobal.php?mes=$mes_value&anno=$anno')\">$neto</a>  </td>";						
		echo "</tr>"; 
	...
	mysql_close();	
		...
		while($row = mysql_fetch_array($_pagi_result)){
			...
			echo "<td align=\"left\"> <a class=enlace href=\"#\" onclick=\"window.open('../librerias/detallePreventivoGlobal.php?sitio=$row[14]&mes=$mes&anno=$anno')\">$row[13]</a>  </td>";		
		...
		echo "<td align=\"left\"> <a class=enlace href=\"#\" onclick=\"window.open('../librerias/detallePreventivoGlobal.php?mes=$mes&anno=$anno')\">$neto</a>  </td>";						
		echo "</tr>"; 
