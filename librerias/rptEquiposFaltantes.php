...
<script language="javascript">
	...
	function generarExcel(){			
			...
			window.open('../librerias/rptEquiposFaltantesExcel.php?anno='+anno+"&sitio="+sitio);		
	}
	
	function generarReporte(){			
			anno=document.frmEquipo.anno.value;			
			sitio=document.frmEquipo.sitio.value;
			window.open('../librerias/rptResumenEquiposFaltantes.php?anno='+anno+"&sitio="+sitio);		
	}
</script>
...
<?php 
	...
	echo "<table class=\"formularioTabla\"align=center width=\"500\" border=\"0\">";
		...
		echo "<tr>";
		echo "<td class=\"tituloPagina\" colspan=\"2\">PREVENTIVO - REPORTE DE EQUIPOS FALTANTES</td>
  				</tr>";
			echo "<tr>";
...
<?php
	...
	GROUP BY configuracion";	
		...
		$_pagi_propagar = array("anno","sitio","item","ordenado","ordentipo","meth");
		...
		while($row = mysql_fetch_array($_pagi_result)){
			...
			for ($j = 1; $j < $count; $j++)
				...
				echo "<td align=\"left\"> <a class=enlace href=\"#\" onclick=\"window.open('../librerias/detalleEquiposPendientesExcel.php?inventario=$row[0]')\">$row[$j]</a>  </td>";
				$neto+=($row[$j]+0);
				}
			echo "</tr>";
			$i++;
		...
		echo "<td align=\"left\"> <a class=enlace href=\"#\" onclick=\"window.open('../librerias/detalleEquiposPendientesExcel.php?sitio=$row[0]&anno=$anno&total=$_pagi_cuantos&pag=$_GET[_pagi_pg]')\">$neto
		</a> </td>";					
		echo "</tr>";
