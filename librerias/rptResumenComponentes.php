...
//echo "funcion: ".$_GET[idDescripcion];
   ...
   //  $rptComponentes= new rptComponentes();   
	...
	//echo 
	$_pagi_sql = "SELECT * FROM (SELECT vistacomponentes.*,(SELECT COUNT(*) FROM planillas_componente WHERE planillas_componente.SERIAL=vistacomponentes.SERIAL) as cp FROM vistacomponentes INNER JOIN modelo ON vistacomponentes.ID_MODELO=modelo.ID_MODELO INNER JOIN TIPO ON modelo.ID_TIPO=tipo.ID_TIPO
	 WHERE $rangoFecha vistacomponentes.SERIAL like '%$_GET[txtSerial]' and vistacomponentes.ID_GERENCIA like '%$_GET[idGerencia]' and vistacomponentes.ID_DESCRIPCION like '%$_GET[idDescripcion]' and vistacomponentes.ID_MARCA like '%$_GET[idMarca]' and
				  ...
				  vistacomponentes.ID_INVENTARIO like '%$_GET[idInventario]' $consitio $conFicha $orden) as temporal";
	...
	//Ordenar Cantidad de Planillas
	
	if ($_GET[ordenado]=='cp' && ($_GET[ordentipo]=='desc'))
		echo "<td valign=top class=\"tablaTitulo\"><a class=\"enlace\" href=\"$_pagi_enlace ordenado=cp&ordentipo=asc\"><b>PLANILLAS DE ASIGNACION</b></a></td>";
	else 
		echo "<td valign=top class=\"tablaTitulo\"><a class=\"enlace\" href=\"$_pagi_enlace ordenado=cp&ordentipo=desc\"><b>PLANILLAS DE ASIGNACION</b></a></td>";
