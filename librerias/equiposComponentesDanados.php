...
switch($funcion) {
	...
	switch($i) {
		case 0:		
		$desperfecto=new inventario("",$_POST[txtSerial],"","","","","");
		...
		if ($row2[20]>=$fechaAsociacion && $row2[21]!='PRO0000000' && $_POST[selInventarioEstado]=='EST0000002'){				 	
			...
			$garantia=$idConse2->retornar();						
		 	$garantinsert= "insert into garantia(ID_GARANTIA,ID_INVENTARIO,ID_ESTATUS_GARANTIA, FECHA_REPORTADO, FECHA_SALIDA, FECHA_FUERA_PLANTA, FECHA_REMPLAZO)
		 	value ('$garantia','$row2[0]','STG0000001','$fechaAsociacion','','','')";		 		 	
		 	echo $garantinsert;
		 	$result3 = mysql_query($garantinsert);		
		}		
		if ($_POST[selInventarioEstado]=='EST0000001'){			
		   $deleteConsulta= "delete from garantia where id_inventario='$row2[0]'";
		   $ejecutaDeleteGarantia=mysql_query($deleteConsulta);		   
		} 				
		if ($_POST[selInventarioEstado]!=$row2[18]||$_POST[selSitio]!=$row2[10]||$_POST[selGerencia]!=$row2[12]||
			$_POST[selDivision]!=$row2[14]||$_POST[selDepartamento]!=$row2[16]){
				...
				VALUES ('$idInvenPropiedad','$row2[0]','$_POST[selInventarioEstado]','$login','$ubicacion','$row2[19]','$fechaAsociacion')";
				echo "<br>otra<br>$consul1<br>";
