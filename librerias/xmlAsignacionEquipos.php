...
$conComponentesAsociados="select * from vistacomponentesasociadosequipos where configuracion='$configuracion' and status_actual=1";
//mysql_close();
...
$filename = '../formularios/asignacionEquipo.rtf';
$fp = fopen ( $filename, 'r' );
...
$output = str_replace( "#UBICACION#", strtoupper( $row[36] ), $output );
$output = str_replace( "#ACTIVO#", strtoupper( $row[14] ), $output );
$output = str_replace( "#CONF#", strtoupper( $row[13] ), $output );
...
$output = str_replace( "#AVANZADAS#", strtoupper( $avanzadas ), $output );	
	...
	$output = str_replace( "#ANALISTA#", strtoupper( $row[49].' '.$row[50] ), $output );	
