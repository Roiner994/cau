...
switch($funcion) {	
	...
	switch($i) {
		...
		$sql="UPDATE descripcion set costo_unitario='$_POST[txtCosto]' where descripcion.id_descripcion='$_POST[selHardware]'";		
		mysql_query($sql);
		if ($resultado && $sql){
				echo "<br><br>";				
				echo "<table class=\"mensajeTitulo\" align=center width=\"500\" border=\"0\">";
...
$conHardware="select distinct id_descripcion,descripcion from descripcion ORDER BY descripcion";
$conCostoUnitario="select distinct costo_unitario from descripcion where id_descripcion='$_POST[selHardware]'";
conectarMysql();
$consulta=mysql_query($conCostoUnitario);
$arreglo=mysql_fetch_array($consulta);
$conEstatus="select id_Motivo_Solicitud,Motivo_Solicitud from motivo_solicitud
			 where status_activo ='1' and id_Motivo_Solicitud='MOT0000001' or id_Motivo_Solicitud='MOT0000004' or id_Motivo_Solicitud='MOT0000005'
	...
	$hardware= new campoSeleccion("selHardware","formularioCampoSeleccion","$_POST[selHardware]","onChange","entrarDenuevo()",$conHardware,"--SELECCIONE--","");
	...
	$txtCosto= new campo("txtCosto","text","formularioCampoTexto","$arreglo[0]","","","onKeyPress","return Num(event)");	
	//$_POST[txtCosto]
	$costoUnitario=$txtCosto->retornar();
	//$costoUnitario=$arreglo[0];			 			
echo "<form name=\"frmRequrimientoHardware\" method=\"post\" action=\"\">";
...
COSTO UNITARIO<br>$ $costoUnitario<br>
</td>
</tr>";
	 	echo "<tr>";
