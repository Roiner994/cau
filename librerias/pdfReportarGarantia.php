...
where inventario.serial like '%' and proveedor.id_proveedor='$garantiaProv'
and garantia_estado.id_estatus_garantia like '%' 
and GARANTIA_ESTADO.STATUS_ACTIVO='1'";
require_once("conexionsql.php");
conectarMysql();
$result=mysql_query($consulta);
$var="REPORTE ESTADISTICO DEL PROVEEDOR: $proveedor";
...
$header=array('SERIAL','DESCRIPCION','FECHA NOTIFICACION','FECHA FUERA DE PLANTA','FECHA REINTEGRO','TIEMPO DE RESPUESTA');
$swiche=0;
while ($row=mysql_fetch_array($result)){ 
	   $fecha=substr($row[14],8,2).'/'.substr($row[14],5,2).'/'.substr($row[14],0,4); 	  	   	   
	   $fecha1=substr($row[15],8,2).'/'.substr($row[15],5,2).'/'.substr($row[15],0,4).' ('.$row[22].')'; 	  	   	   
	   $fecha2=substr($row[16],8,2).'/'.substr($row[16],5,2).'/'.substr($row[16],0,4).' ('.$row[23].')'; 	  	   	   
	   $pdf->Row($header,array($row[2],$row[6],$fecha,$fecha1,$fecha2,$row[24]),$swiche);
	   $swiche=1;
}
$pdf->Output();
?> 