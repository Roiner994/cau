					...
					switch ($resultado) {
						case 0:
							...
							if (isset($_POST[txtConfiguracion]) && !empty($_POST[txtConfiguracion])) {
								echo "<input name=\"btn\" type=\"button\" value=\"IMPRIMIR\" onClick=\"window.open('../librerias/xmlAsignacionComponentes.php?idDespacho=$idDespacho&config=$_POST[txtConfiguracion]&ficha=$_POST[txtFicha]')\"></td>";
							} else {
								echo "<input name=\"btn\" type=\"button\" value=\"IMPRIMIR\" onClick=\"window.open('../librerias/xmlAsignacionComponentes.php?idDespacho=$idDespacho&ficha=$_POST[txtFicha]')\"></td>";
							}
							echo "</tr>";
							echo "</table>";
							echo "</form>";	
...
function frmDespacho($elementos="",$quitar="") {
	...
	$conDescripcion="Select
		...
		descripcion
		Where
		descripcion.ID_DESCRIPCION_PROPIEDAD = 'DSP0000008' OR descripcion.ID_DESCRIPCION='DES0000022'
		Order By
...
echo "<input name=\"despachar\" type=\"hidden\" value=\"$_POST[despachar]\">";
	...
	echo "<tr>";
	echo "<td class=\"tituloPagina\" colspan=\"2\">DESPACHO DE COMPONENTES PARA IMPRESORAS</td>
	</tr>";
	echo "<tr>
...
echo "<table class=\"formularioTabla\"align=center width=\"40%\" border=\"0\">";
	...
	echo "</table>";
			...
			$resultadoBuscar=$componente->buscarComponentesDisponiblesImpresoras();
			$despachado= new despacho();
			if ($resultadoBuscar && $resultadoBuscar!=1 && ($_POST[selDescripcion]!="" || $_POST[txtSerialDisponible]!="")) {
	...
	CONFIGURACION<br>
	<input name=\"txtConfiguracion\" value=\"$_POST[txtConfiguracion]\" class=\"formularioCampoTexto\"></td>
	<td valign=top class=\"formularioCampoTitulo\">
	FICHA<br>
