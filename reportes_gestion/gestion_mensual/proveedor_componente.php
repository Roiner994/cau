...
$_pagi_propagar = array("selproveedor","id_pedido");//No importa si son POST o GET
...
if ($_GET[selproveedor]==100) {

 $_GET[selproveedor]= "";

}
$conGerencia="select id_descripcion,
id_proveedor,date_format(fecha_inicio,'%d/%m/%Y') as Inicio,
proveedor,descripcion,count(*) as CANTIDAD,ID_PEDIDO 
from vistacomponentes  
where id_proveedor like '%$_GET[selproveedor]%' group by  ID_PEDIDO,id_descripcion  ";
...
<div id="cuerpo">		
<h2>REPORTES DE PROVEEDORES POR EQUIPOS</h2>
<?php

	if  ($_pagi_result &&mysql_num_rows($_pagi_result)>0); { ?>	
		<table align=center border=1 cellpadding=0 cellspacing=0 borercolor width="90%"%"%">
...
<td><a href='componente_proveedor.php?idpedido=<?=$rowGerencia[ID_PEDIDO]?>&iddescripcion=<?=$rowGerencia[ID_DESCRIPCION]?>'><img src="../imagenes/lupas.jpg" width="23" height="15"style=border:0></a></td>
...
<td class="foottotal" colspan="5">TOTAL</td>
		<td><?=$total?></td>
</tbody>
</table>
</div>
...
<p class="paginador"><?=$_pagi_navegacion?></p>
	...
	<a href="proveedorcomponentexcel.php?selproveedor=<?=$_GET[selproveedor]?>&amp;id_pedido">FORMATO EXCEL</a></p>