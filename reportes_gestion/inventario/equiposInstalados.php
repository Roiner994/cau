...
switch ($_GET[orden]) {
	...
	$_pagi_sql = "select ID_SITIO,SITIO,
...
<div id="contenedor">
	...
	if ($_pagi_result && mysql_numrows($_pagi_result)>0) { ?>
		...
		<th><a href="equiposinstalados.php?orden=microcomputador&modo=<?=$_GET[modo]?>">MICROCOMPUTADORES</a></th>
		<th><a href="equiposinstalados.php?orden=laptop&modo=<?=$_GET[modo]?>">LAPTOP</a></th>
		<th><a href="equiposinstalados.php?orden=impresora&modo=<?=$_GET[modo]?>">IMPRESORAS</a></th>
	...
	<?php
		while ($row=mysql_fetch_array($_pagi_result)) {
			...
			<td onclick="location.href='equipoinstaladoslista.php?idSitio=<?=$row[0]?>'"><?=$row[1]?></td>
			<td onclick="location.href='equipoinstaladoslista.php?idSitio=<?=$row[0]?>&idDescripcion=DES0000001'"><?=$row[2]?></td>
			<td onclick="location.href='equipoinstaladoslista.php?idSitio=<?=$row[0]?>&idDescripcion=DES0000042'"><?=$row[3]?></td>
			<td onclick="location.href='equipoinstaladoslista.php?idSitio=<?=$row[0]?>&idDescripcion=DES0000008'"><?=$row[4]?></td>
			<td onclick="location.href='equipoinstaladoslista.php?idSitio=<?=$row[0]?>'"><?=$row[5]?></td>
			</tr>
		<?php 
		} ?>
	</ul>
